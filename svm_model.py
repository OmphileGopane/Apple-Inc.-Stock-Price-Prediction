...
X = train_data[:, 1:]  # Features (excluding 'Close/Last' if it's in features)
y = train_data[:, 0]   # Target variable (assuming 'Volume' is the target variable)

# Split data into train and test sets (if not using sequences)
...
plt.title('Stock Price Prediction (SVM)')
plt.xlabel('Time')
